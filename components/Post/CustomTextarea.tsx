...
export default function CustomTextarea({ placeholder, className, maxLength, value, onChange }: TweetAreaProps) {
	...
	const handleChange = (e: React.ChangeEvent<HTMLDivElement>) => {
		...
		if (match) {
			match.forEach((m) => {
				...
				span.textContent = m;
				span.onclick = () => window.open(`/@${m.substring(1)}`, '_blank');
				content = content.replace(m, span.outerHTML);
			});
		}
